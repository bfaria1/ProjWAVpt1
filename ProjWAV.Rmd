...
```{r}
grafico <- ggplot(dados, aes(x = C1, y = C2, color = L1, alpha =I(.5)))+
  geom_point()+
  scale_colour_manual(values = c("red", "blue","green")) +
  stat_smooth(method = "lm",
              formula = y~poly(x,2),
...
A1 <- hist(R1, 
           plot = FALSE, 
           breaks = c(-0.4,-0.275,-0.25,-0.225,-0.2,-0.175,-0.15,-0.125,-0.1,-0.075,-0.05,-0.025,0,0.025,0.05,0.075,0.1,0.125,0.15,0.172,0.2))
A1$density = A1$counts/sum(A1$counts)*100
...
dados <- data.frame(L = A, x = B, y = C)
 ...
 ggplot(dados, aes(x = x, y = C, group = L, colour = L))+
  scale_colour_manual(values = c("red", "blue", "green")) +
  stat_smooth(method = lm, formula = y ~ poly(x, 10),se = FALSE, fullrange = TRUE)+
  labs(x = "Número de Espécies (Log10)",
       y = "Frequência Relativa (%)",
       colour = " ")+
  theme_classic()
...
#ggarrange(grafico, tatu,  ncol = 2, nrow = 1, common.legend = TRUE, legend = "right")
```
